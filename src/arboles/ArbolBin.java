        ...
        do { 
            ...
            switch (opc) { 
                ...
                case 13: 
                    ...
                    break;
case 14:
String des = JOptionPane.showInputDialog("Ingrese la letra a la que le buscaremos sus desendiente:"); 
                    char des = des.charAt(0); 

                    arbol.cyd(arbol.raiz, des); 
break: 
...
return;
    ...
    if (r.getDato() == descendiente) {
        int cantidad = r.getLI()+ r.getLD()
        System.out.println("Cantidad de descendientes: " + cantidad);
        return;
    }

    if (descendiente < r.getDato()) {
        cymd(r.getLI(), descendiente);
    } else {
        cymd(r.getLD(), descendiente);
    }
}